...
- `src/components/about/CircuitScene.tsx` *(new, client component, `dynamic(() => import(...), { ssr: false })`)* — the `<Canvas>`. Loads the CC0 models via `useGLTF` + `Suspense`, sets up ambient + directional lighting (directional light recolors per active theme, same `getComputedStyle` + `MutationObserver` pattern already used in `VoxelScene.tsx`), renders the camera rig, and includes the diagnostic probe mesh (kept in this same file per user preference — no separate component file for now). Exposes one prop: `progress: number` (0–1), driving camera position/lookAt along a hand-authored 4-stop path.
...
- No changes to the homepage hero (`VoxelScene.tsx`) — pattern is reused for consistency, not modified.
